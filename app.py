...
with tab3:
    ...
    beeswarm_path = os.path.join(BASE_DIR, 'plots', 'shap_summary_beeswarm.png')
    importance_path = os.path.join(BASE_DIR, 'plots', 'shap_feature_importance.png')
    
    with col_g1:
        st.markdown("#### Global SHAP Summary (Beeswarm Plot)")
        if os.path.exists(beeswarm_path):
            st.image(beeswarm_path, caption="Impact of features across the entire patient cohort")
        
    with col_g2:
        st.markdown("#### Mean Absolute SHAP Importance Ranking")
        if os.path.exists(importance_path):
            st.image(importance_path, caption="Top predictive clinical biomarkers ranked by mean |SHAP|")
...
with tab4:
    ...
    *BMC Medical Informatics and Decision Making (2025) 25:162 (Awe et al.)*
    ...
    bench_csv = os.path.join(BASE_DIR, 'plots', 'benchmark_comparison.csv')
    comp_bar = os.path.join(BASE_DIR, 'plots', 'model_comparison_bar.png')
    roc_plot = os.path.join(BASE_DIR, 'plots', 'roc_auc_curves.png')
    ...
    with col_p1:
        if os.path.exists(comp_bar):
            st.image(comp_bar, caption="Accuracy Comparison: Our Stacking Ensemble vs Paper Baseline")
    with col_p2:
        if os.path.exists(roc_plot):
            st.image(roc_plot, caption="Receiver Operating Characteristic (ROC-AUC) Curves")
